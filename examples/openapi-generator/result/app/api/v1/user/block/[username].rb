...
# /user/block/{username} -- scaffolded from the document.
...
RSpec.describe "/user/block/{username}", type: :openapi do
  openapi_schema :public_api

  api_path "/user/block/{username}" do
    parameter name: :username, in: :path, schema: {"type" => "string"}, required: true

    put "Blocks a user from the doer" do
      tags "user"
  ...
  it "PUT /api/v1/user/block/{username} answers 204" do
