...
# builder — this test proves that path survives the frontend intact.
class SinatraFrontendTest < Minitest::Test
  ...
  def setup
    unless ADAPTER_AVAILABLE
      skip "ratalada/sinatra is not in the active bundle"
    end
  end
  ...
  def test_middleware_runs_around_the_route
    ...
    assert_equal "sinatra-ran", headers["X-Middleware"]
  ...
  def build(&block)
    Ratalada::Frontends::Sinatra.build(block)
  end
  ...
  def header_tagging_middleware
    Class.new do
      ...
      def call(env)
        ...
        headers["X-Middleware"] = "sinatra-ran"
        [status, headers, body]
      end
    end
  end
